...
import type { IFileSystemService } from '../services/filesystem';
...
export class ExecutionRepository implements IExecutionRepository {
  ...
  constructor(scoreAnalysisService: ScoreAnalysisService, fileSystem: IFileSystemService) {
  ...
  async save(execution: TestExecution): Promise<void> {
    ...
    await this.fileSystem.writeFile(filePath, JSON.stringify(validated.data, null, 2), 'utf-8');
  }
  ...
  async findTestCaseResult(id: string, seed: number): Promise<string | null> {
    ...
    try {
      return await this.fileSystem.readFile(outputFilePath, 'utf-8');
    } catch (err: unknown) {
      if ((err as NodeJS.ErrnoException)?.code === 'ENOENT') return null;
      throw err;
    }
  ...
  async delete(id: string): Promise<void> {
    try {
      // TODO: Implement recursive directory deletion in IFileSystemService
      throw new Error('Delete functionality not yet implemented with file system abstraction');
    } catch (error) {
